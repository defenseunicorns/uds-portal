...
import ResourceLink from '$components/k8s/ResourceLink/component.svelte'
...
interface Row extends CommonRow {
  ...
  node: { component: typeof ResourceLink; props: { name: string; route: string; resourcePath: string } }
...
export function createStore(): ResourceStoreInterface<Resource, Row> {
  ...
  const transform = transformResource<Resource, Row>((r) => ({
    ...
    node: {
      ...
      props: {
        name: r.spec?.nodeName ?? '',
        route: 'nodes',
        resourcePath: '/api/v1/resources/nodes',
      },
    },
  }))
