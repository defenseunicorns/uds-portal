...
// Vitest type redeclared because it's not exported from vitest
...
export function testK8sTableWithDefaults(Component: ComponentType, props: Record<string, unknown>) {
  test('creates component with correct props and default columns', () => {
    ...
    // add drawerOpts if provided
    if ('drawerOpts' in props) {
      const drawerOpts = props.drawerOpts as DrawerOpts
      expectedProps.drawerOpts = expect.objectContaining({
        includeEvents: drawerOpts.includeEvents,
...
export class MockResourceStore {
  ...
  // added to satisfy optional restart method
  restart() {}
