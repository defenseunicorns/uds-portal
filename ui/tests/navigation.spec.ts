...
test.describe('Navigation', async () => {
  ...
  test('Overview page', async ({ page }) => {
    ...
    const pkgCountEl = (await page.getByTestId('resource-count-packages').textContent())!

    // + converts string to number
    expect(+pkgCountEl).toBeGreaterThanOrEqual(1)

    const card = page.getByTestId('card-container')

    await expect(card.getByText('Pods Running')).toBeVisible()
    await expect(card.getByText('Packages Deployed')).toBeVisible()
    ...
    // Check for the Core Services Widget
    await expect(page.getByRole('heading', { name: 'Core Services' })).toBeVisible()

    // Check for the Applications Widget
  ...
  test('Navigate to page when click on Pod and Package Cards', async ({ page }) => {
    const podsCard = page.getByTestId('card-container').filter({ hasText: 'Pods Running' })
    await podsCard.click()
    expect(await page.getByTestId('table-header').textContent()).toEqual('Pods')

    await page.goto('/')

    const packagesCard = page.getByTestId('card-container').filter({ hasText: 'Packages Deployed' })
    await packagesCard.click()
    expect(await page.getByTestId('table-header').textContent()).toEqual('UDS Packages')
  })
